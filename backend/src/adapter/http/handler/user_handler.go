...
// 		return c.JSON(http.StatusOK, interfaces.NewAPIResponse(conf.ErrBadRequest, err.Error(), nil))
// 	}
// 	user, err := h.UserUseCase.GetUser(ctx, uint(id))
// 	if err != nil {
...
// 		}
// 		return c.JSON(http.StatusOK, interfaces.NewAPIResponse(code, err.Error(), nil))
// 	}
// 	return c.JSON(http.StatusOK, interfaces.NewAPIResponse(0, interfaces.StatusText(interfaces.StatusSuccess), interfaces.ToUserResponse(user)))
// }
...
// 	req := &interfaces.UserSearchRequest{}
...
// 	// TODO：状況に応じて変更予定
// 	inputdata := &iodata.UserSearchInputData{
// 		UserID: req.UserID,
// 	}

// 	// 複数案件の取得
// 	users, err := h.UserUseCase.GetUsers(ctx, inputdata)
// 	if err != nil {
...
// 		return c.JSON(http.StatusOK, interfaces.NewAPIResponse(code, err.Error(), nil))
// 	}
// 	return c.JSON(http.StatusOK, interfaces.NewAPIResponse(0, interfaces.StatusText(interfaces.StatusSuccess), interfaces.ToUserResponseList(users).Users))
// }
...
// 	// HTTP request bodyのbind処理
// 	req := &interfaces.UserDeleteRequest{}
// 	if err := c.Bind(req); err != nil {
// 		zap.S().Errorw("failed to bind", zap.Error(err))
// 		err = conf.NewAppError(conf.ErrBadRequest, err).Wrap()
// 		return c.JSON(http.StatusOK, interfaces.NewAPIResponse(conf.ErrBadRequest, err.Error(), nil))
...
// 		return err
// 	}
// 	inputdata := &iodata.UserDeleteInputData{
// 		ID:        uint(id),
// 		UpdatedAt: updatedAt,
// 	}

// 	if err := h.UserUseCase.DeleteUser(ctx, inputdata); err != nil {
...
// 		return c.JSON(http.StatusOK, interfaces.NewAPIResponse(code, err.Error(), nil))
// 	}
// 	return c.JSON(http.StatusOK, interfaces.NewAPIResponse(0, interfaces.StatusText(interfaces.StatusSuccess), nil))
// }
